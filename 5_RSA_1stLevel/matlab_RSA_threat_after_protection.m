...
for ind_subject = 1
    for ind_run = 1:number_of_run
        ...
        index_target = find(cellfun(@(x,y) strcmp(x, 'protection') & strcmp(y, 'threat'), labels(:,3), labels(:,5)));
            ...
            for col = 1:count_trialtype
                for row = 1:count_trialtype
                    if(row > col)
                        ...
                        % stim 1, protection
                        if (strcmp(labels{col, 3}, 'protection')  & strcmp(labels{row, 3}, 'protection') & (labels{col, 4}==labels{row, 4}))
                            biary_matrix(counter, 1) = 1;
                        end
                        % stim 1, threat
                        if (strcmp(labels{col, 5}, 'threat')  & strcmp(labels{row, 5}, 'threat') & (labels{col, 6}==labels{row, 6}))
...
for ind_subject = 1
    for ind_run = 1:number_of_run
        ...
        MRIwrite(beta_images{1,1},[root_path,  'sarah_safetyEstimationTask/outputs/result_rsa_secondstim_threat_after_protection/protec_sub',num2str(ind_subject), '_run', num2str(ind_run),'.nii']);

        beta_images{1,1}.vol = volume_threat;
        MRIwrite(beta_images{1,1},[root_path,  'sarah_safetyEstimationTask/outputs/result_rsa_secondstim_threat_after_protection/threat_sub',num2str(ind_subject), '_run', num2str(ind_run),'.nii']);

        beta_images{1,1}.vol = volume_outcome;
        MRIwrite(beta_images{1,1},[root_path,  'sarah_safetyEstimationTask/outputs/result_rsa_secondstim_threat_after_protection/outcome_sub',num2str(ind_subject), '_run', num2str(ind_run),'.nii']);

        beta_images{1,1}.vol = volume_shock;
        MRIwrite(beta_images{1,1},[root_path,  'sarah_safetyEstimationTask/outputs/result_rsa_secondstim_threat_after_protection/shock_sub',num2str(ind_subject), '_run', num2str(ind_run),'.nii']);
