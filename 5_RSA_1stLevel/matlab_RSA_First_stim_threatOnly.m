...
for ind_subject = 1
    for ind_run = 1:number_of_run
        ...
        index_target = find(cellfun(@(x) strcmp(x, 'threat'), labels(:,3)));
...
for ind_subject = 1
    for ind_run = 1:number_of_run
        ...
        MRIwrite(beta_images{1,1},[root_path,  'sarah_safetyEstimationTask/outputs/result_rsa_firstStim_threatOnly/protec_sub',num2str(ind_subject), '_run', num2str(ind_run),'.nii']);

        beta_images{1,1}.vol = volume_threat;
        MRIwrite(beta_images{1,1},[root_path,  'sarah_safetyEstimationTask/outputs/result_rsa_firstStim_threatOnly/threat_sub',num2str(ind_subject), '_run', num2str(ind_run),'.nii']);

        beta_images{1,1}.vol = volume_outcome;
        MRIwrite(beta_images{1,1},[root_path,  'sarah_safetyEstimationTask/outputs/result_rsa_firstStim_threatOnly/outcome_sub',num2str(ind_subject), '_run', num2str(ind_run),'.nii']);

        beta_images{1,1}.vol = volume_shock;
        MRIwrite(beta_images{1,1},[root_path,  'sarah_safetyEstimationTask/outputs/result_rsa_firstStim_threatOnly/shock_sub',num2str(ind_subject), '_run', num2str(ind_run),'.nii']);
