...
for ind_subject = 1
    for ind_run = 1:number_of_run
        ...
        % retrieve label
        labels = matlab_return_label_1st_stim(root_path, ind_subject, ind_run);
...
for ind_subject = 1
    for ind_run = 1:number_of_run
        ...
        MRIwrite(beta_images{1,1},[root_path,  'sarah_safetyEstimationTask/outputs/result_rsa_firstStim/protec_sub',num2str(ind_subject), '_run', num2str(ind_run),'.nii']);

        beta_images{1,1}.vol = volume_threat;
        MRIwrite(beta_images{1,1},[root_path,  'sarah_safetyEstimationTask/outputs/result_rsa_firstStim/threat_sub',num2str(ind_subject), '_run', num2str(ind_run),'.nii']);

        beta_images{1,1}.vol = volume_outcome;
        MRIwrite(beta_images{1,1},[root_path,  'sarah_safetyEstimationTask/outputs/result_rsa_firstStim/outcome_sub',num2str(ind_subject), '_run', num2str(ind_run),'.nii']);

        beta_images{1,1}.vol = volume_shock;
        MRIwrite(beta_images{1,1},[root_path,  'sarah_safetyEstimationTask/outputs/result_rsa_firstStim/shock_sub',num2str(ind_subject), '_run', num2str(ind_run),'.nii']);
